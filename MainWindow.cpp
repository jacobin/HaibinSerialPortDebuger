    ...
    serial_port(new QSerialPort(this)), ui(new Ui::MainWindow)
{
    ...
    {
        MessageBox::about(this, QStringLiteral("关于"), QStringLiteral("本软件由海滨开发且开放源代码，遵循LGPL协议。"));
    });
    ...
    QObject::connect(this->ui->rB_rcvMode_text, &QRadioButton::toggled, [this](bool is_checked)
    {
        if(this->rcv_data_map->size() > 0)
        {
            ...
            foreach(qint64 unix_timestamp, this->rcv_data_map->keys())
            {
                ...
                if(type == Serial_RCV)
                {
                    rcvStr += QStringLiteral("[收] ");
                }
                else
                {
                    rcvStr += QStringLiteral("[发] ");
                }
    ...
    QObject::connect(this->serial_port, &QSerialPort::readyRead, [this]
    {
        ...
        QString rcvStr(QStringLiteral("[收] "));
        rcvStr += QDateTime::fromMSecsSinceEpoch(unix_timestamp).toString("yyyy-MM-dd hh:mm:ss zzz");
        rcvStr += "\r\n";
    ...
    QObject::connect(ui->pB_openSerail, &QPushButton::clicked, [this]
    {
        if(this->is_open_serailport)
        {
            ...
            if(this->ui->pB_openSerail->text() == QStringLiteral("关闭"))
            {
                this->ui->pB_openSerail->setText(QStringLiteral("打开"));
            }
        ...
        else
        {
            ...
            if(!ret)
            {
                ...
                MessageBox::warning(this, QStringLiteral("设置波特率失败"), QStringLiteral("原因:") + serial_port->errorString());
                return;
            }
            // 获取数据位
            ...
            if(!ret)
            {
                ...
                MessageBox::warning(this, QStringLiteral("设置数据位失败"), QStringLiteral("原因:") + serial_port->errorString());
                return;
            }
            // 获取停止位
            ...
            if(!ret)
            {
                ...
                MessageBox::warning(this, QStringLiteral("错误"), QStringLiteral("设置停止位失败，失败原因：") + serial_port->errorString());
                return;
            }
            // 获取校验位
            ...
            if(!ret)
            {
                ...
                MessageBox::warning(this, QStringLiteral("设置校验位失败"), QStringLiteral("原因:") + serial_port->errorString());
                return;
            }
            // 设置流控
            ...
            if(!ret)
            {
                ...
                MessageBox::warning(this, QStringLiteral("设置流控失败"), QStringLiteral("原因:") + serial_port->errorString());
                return;
            }
            // 设置读写
            ...
            if(ret)
            {
                this->ui->pB_openSerail->setText(QStringLiteral("关闭"));
                if(this->ui->cB_serailport->isEnabled())
                {
            ...
            else
            {
                ...
                MessageBox::warning(this, QStringLiteral("打开串口失败"), QStringLiteral("原因:") + serial_port->errorString());
            }
        }
    });
    ...
    QObject::connect(ui->pB_autoSend, &QPushButton::clicked, [this]
    {
        ...
        if(this->ui->pB_autoSend->text() == QStringLiteral("自动发送"))
        {
            bool is_ok = false;
...
//                this->send_timer->setInterval(send_inter_ms);
                ...
                this->ui->pB_autoSend->setText(QStringLiteral("停止发送"));
            ...
            {
                qDebug() << "报错";
                MessageBox::warning(this, QStringLiteral("错误"), QStringLiteral("请查看串口是否打开以及时间间隔和内容是否设置！"));
            }
        ...
        else
        {
            ...
            this->ui->pB_autoSend->setText(QStringLiteral("自动发送"));
    ...
    QObject::connect(this->ui->pB_sendfile, &QPushButton::clicked, [this]
    {
        ...
        if(!fileName.isEmpty())
        {
            ...
            if(send_buffer.length() > 0)
            {
                ...
                QString sendStr(QStringLiteral("[发] "));
                ...
                QScrollBar* scrollBar = this->ui->tE_rcvcon->verticalScrollBar();
                scrollBar->setValue(scrollBar->maximum());
                MessageBox::info(this, QStringLiteral("成功"), QStringLiteral("发送文件成功"));
            }
    ...
    QObject::connect(ui->cB_bautrate, &QComboBox::currentTextChanged, [this](QString)
    {
        if(this->is_open_serailport)
        {
            ...
            if(!ret)
            {
                ...
                MessageBox::warning(this, QStringLiteral("错误"), QStringLiteral("切换失败，可能当前硬件不支持这种波特率！"));
            }
        }
    });
}
    ...
    QObject::connect(ui->cB_databits, &QComboBox::currentTextChanged, [this](QString)
    {
        if(this->is_open_serailport)
        {
            ...
            if(!ret)
            {
                ...
                MessageBox::warning(this, QStringLiteral("错误"), QStringLiteral("切换失败，可能当前硬件不支持这种数据位！"));
//                return false;
            }
        }
    });
...
void MainWindow::initStopBitList()
{
    ...
    QObject::connect(ui->cB_stopbit, &QComboBox::currentTextChanged, [this](QString)
    {
        if(this->is_open_serailport)
        {
            ...
            {
                qDebug() << "切换失败";
                MessageBox::warning(this, QStringLiteral("错误"), QStringLiteral("切换失败，可能当前硬件不支持这种停止位！"));
...
void MainWindow::initCheckBitList()
{
    ...
    checkbit_list.insert(QStringLiteral("无校验"), QSerialPort::NoParity);
    checkbit_list.insert(QStringLiteral("奇校验"), QSerialPort::OddParity);
    checkbit_list.insert(QStringLiteral("偶校验"), QSerialPort::EvenParity);
    checkbit_list.insert(QStringLiteral("1 校验"), QSerialPort::MarkParity);
    checkbit_list.insert(QStringLiteral("0 校验"), QSerialPort::SpaceParity);
    foreach(const QString& key, checkbit_list.keys())
    {
        ui->cB_checkbit->addItem(key, checkbit_list[key]);
    }
    ui->cB_checkbit->setCurrentText(QStringLiteral("无校验"));
    ...
    QObject::connect(ui->cB_checkbit, &QComboBox::currentTextChanged, [this](QString)
    {
        if(this->is_open_serailport)
        {
            ...
            {
                qDebug() << "切换失败";
                MessageBox::warning(this, QStringLiteral("错误"), QStringLiteral("切换失败，可能当前硬件不支持这种校验位！"));
...
void MainWindow::slotOnSaveData()
{
    ...
    if(!fileName.isEmpty())
    {
        ...
        file.close();
        MessageBox::info(this, QStringLiteral("成功"), QStringLiteral("保存文件成功"));
    }
}
